...
try {
    ...
    for (const p in chart.planetaryDetails) {
        console.log("   " + p + ": " + printSign(chart.planetaryDetails[p].longitude) + " " + chart.planetaryDetails[p].longitude.toFixed(2));
    }
    
    // Check houses specifically for this logic
    ...
    console.log("\\n---- GAP YEAR DRISHTI ----");
    console.log("Saturn aspects House:", (getHouse('saturn') + 2)%12 || 12, (getHouse('saturn') + 6)%12 || 12, (getHouse('saturn') + 9)%12 || 12);

    console.log("\\n---- GAP YEAR PREDICTION SCORE & OUTPUT ----");
...
fs.writeFileSync('temp-runner3.js', testCode);
