...
class EducationEngine {
    ...
    _educationTimeline(chart) {
        ...
        let schoolNote = 'School years proceed with reasonable consistency. Interest in academics develops gradually, with specific subject areas of fascination emerging by age 10-12.';
        if (satHouse === 4) schoolNote = 'Saturn in 4th may create a challenging schooling environment. Frequent changes of schools or teachers are possible, requiring patience and parental support.';
        else if (rahuHouse === 4 || ketuHouse === 4) schoolNote = 'Rahu or Ketu in the 4th house suggests an unconventional early schooling environment, sudden shifts in learning styles, or frequent relocations.';
        else if (jupHouse === 4 || merHouse === 4) schoolNote = 'A highly favorable foundation for learning. Teachers and mentors will play a huge role in sparking academic curiosity early on.';
        events.push({ age: '6-14', phase: 'School Years', note: schoolNote });

        // 15-18 (Secondary Education - 5th House / Intellect)
        let secondaryNote = 'A crucial period for higher secondary and board exams. Consistency and balanced focus are key during these formative years.';
        if (satHouse === 5) secondaryNote = 'Saturn in the 5th demands intense discipline and can cause stress or slight delays during board exams, but yields solid long-term results.';
        else if (merHouse === 5 || jupHouse === 5) secondaryNote = 'Excellent period for intellectual growth. High performance in board exams and a strong grasp of complex subjects is indicated.';
        else if (rahuHouse === 5) secondaryNote = 'Rahu in the 5th brings sudden shifts in interests. You may consider changing streams (e.g., Science to Commerce/Arts) abruptly.';
        ...
        let undergradNote = 'The prime window for undergraduate studies. Focus shifts towards specializing in a chosen field and building career foundations.';
        if (jupHouse === 9 || getHouse('sun') === 9) undergradNote = 'Outstanding prospects for higher education. High chances of scholarships, admission to prestigious universities, or entering specialized prominent fields.';
        else if (rahuHouse === 9 || rahuHouse === 12) undergradNote = 'Strong indications for pursuing undergraduate studies abroad or in unconventional fields related to technology, foreign languages, or modern sciences.';
        else if (satHouse === 9) undergradNote = 'Saturn in the 9th indicates a hardworking undergraduate phase. Success comes through extreme persistence and perhaps overcoming initial obstacles or a gap year.';
        else if (ketuHouse === 9) undergradNote = 'Ketu in the 9th brings deep research-oriented studies, or conversely, a detachment from traditional college paths leading to unique personalized learning.';
        events.push({ age: '18-24', phase: 'Undergraduate Studies', note: undergradNote });

        // 24-30 (Postgraduate / Professional Studies)
        let postgradNote = 'This phase brings a deepening of professional mastery, often prompting postgraduate degrees (Masters/MBA) or career-focused certifications.';
        if (getHouse('mars') === 10 || getHouse('sun') === 10) postgradNote = 'A strong drive for professional education. Executive MBAs or leadership-oriented certifications are highly favored here to boost career velocity.';
        else if (merHouse === 9 || jupHouse === 9) postgradNote = 'A natural inclination towards a Master\'s degree, PhD, or deep academic research during this period is highly probable.';
        else if (satHouse >= 10) postgradNote = 'Professional life and career responsibilities may take precedence over formal studies, causing postgraduate education to be pursued part-time or delayed slightly.';
        events.push({ age: '24-30', phase: 'Postgraduate / Professional Studies', note: postgradNote });

        // 30+ (Specialized / Continuing Education)
        let contNote = 'Lifelong learning and skill upgrades keep the mind sharp during this mature phase. Often brings a return to studies for personal fulfillment.';
        if (ketuHouse === 8 || ketuHouse === 12) contNote = 'A strong pull towards occult sciences, astrology, or deep spiritual/philosophical learning often begins or matures after age 30.';
        else if (jupHouse !== null) contNote = 'Jupiter\'s influence ensures an eventual return to wisdom-seeking, perhaps taking on teaching, mentoring, or advanced spiritual studies as a second educational journey.';
